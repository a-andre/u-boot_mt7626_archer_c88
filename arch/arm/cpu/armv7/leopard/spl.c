...
void spl_sys_led_init()
{
/* 暂时禁用spl LED代码，目前LED初始化位于uboot中；*/
#if 0
#define GPIO_DIR_BASE 0x10217000
#define GPIO_OUT_BASE 0x10217100
	u32 reg = 0;
	u32 val = 0;

	reg = GPIO_DIR_BASE + (SYS_LED >> 5) * 0x10;
	val = readl(reg);
	val |= (1 << (SYS_LED & 0x1F));
	writel(val, reg);
#ifdef SYS_LED_INV
	reg = GPIO_OUT_BASE + (SYS_LED >> 5) * 0x10;
	val = readl(reg);
	val |= (1 << (SYS_LED & 0x1F));
	writel(val, reg);
#endif
