...
/* In MIPS, CFG_HZ should be CPU_FREQ/2.
 * In ARM with MT7626, CFG_HZ is only used in get_tbclk() and used to calculate endtick() in main.c.
 * #define	endtick(seconds) (get_ticks() + (uint64_t)(seconds) * get_tbclk())
 * get_ticks() return ticks in ms, not real timer ticks. In order to get an accurate result, 
 * we should define CFG_HZ as 1000 here.
*/
#define CFG_HZ              (1000)
