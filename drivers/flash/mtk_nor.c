...
int mtk_nor_read(u32 addr, u32 len, size_t *retlen, u8 *buf)
{
	...
	*retlen = NOR_BUF_LEN - data_header;
	...
	if (ret)
		return ret;
	memcpy(buf + cur_index, nor_dma_buf, addr + len - cur_addr);
	*retlen = len;
	return 0;
}
...
{
#define TST_ADDR 0x101
#define TST_SIZE 0x1008
	int ret, i;
	size_t retlen;
	u8 *buf;
				...
				int argc, char * const argv[])
{
	...
	if (!strncmp(argv[1], "read", 5)) {
		...
		printf("Nor flash read len: 0x%x done.\n", retlen);
		...
		printf("Nor flash erase len: 0x%x done.\n", len);
		...
		printf("Nor flash write len: 0x%x done.\n", retlen);
...
#ifdef CONFIG_UIP
static int tp_erase(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	ulong addr, length;
	int ret;
	int size;
	...
	ret = addr_flash((void*)addr);
	if(ret < 0)
	{
		return ret;
	}

	addr -= gd->bd->bi_flashstart;
	ret = mtk_nor_erase(addr, length);
	if (ret) {
		snfc_loge("Erase Fail\n");
		return ERR_FAULT;
	}
