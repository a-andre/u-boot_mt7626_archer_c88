...
#define GPIO_DRIV(x)        (GPIO_SNAND_BASE + 0x900 + 0x10 * x)
...
static bool mtk_snand_check_bch_error(struct mtd_info *mtd, u8 *databuf, u8 *spareBuf, u32 u4SecIndex, u32 u4PageAddr)
{
		...
		failed_sec = 0;
			...
			if (0x1F == errnum) {
				...
				if (errnum) {
					...
					MSG(INIT, "[%s] ECC-C, #err:%d, PA=%d, S=%d\n", __func__, errnum, u4PageAddr, i);
				}
			}
		}
	...
	mtd->ecc_stats.failed += failed_sec;
		if (correct_count > 2 && ret)
			...
			MSG(INIT, "NFI Less than 2 bit error, ignore\n");
	}
	return ret;
}
...
int mtk_nand_block_bad_hw(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (g_snand_spare[offset] != 0xff) {
		MSG(INIT, "Bad block detected at 0x%x, oob_buf[%d] is 0x%x\n", page_addr, offset, g_snand_spare[offset]);
		return 1;
	}
