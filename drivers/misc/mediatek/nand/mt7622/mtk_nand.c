...
/* Add Default NAND Config */
#define DEFAULT_BLOCKSZ	128
#define DEFAULT_TOTALSZ	128
#define DEFAULT_PAGESZ	2048
#define DEFAULT_ADDRCY	4
#define DEFAULT_TIMING	0x10804211
#define DEFAULT_SPARESZ	64
#define DEFAULT_ACCCON	0xC03222
#define DEFAULT_ACCCON1	0x101
...
bool get_device_info(u8*id, flashdev_info *gn_devinfo)
{
    ...
    if(target != -1){
		...
		MSG(INIT, "]\n");
	    MSG(INIT, "Use default NAND\n");
		gn_devinfo->blocksize = DEFAULT_BLOCKSZ;
		gn_devinfo->addr_cycle = DEFAULT_ADDRCY;
		gn_devinfo->iowidth = IO_8BIT;
		gn_devinfo->timmingsetting = DEFAULT_TIMING;
		gn_devinfo->advancedmode = 0;
		gn_devinfo->pagesize = DEFAULT_PAGESZ;
		gn_devinfo->sparesize = DEFAULT_SPARESZ;
		gn_devinfo->totalsize = DEFAULT_TOTALSZ;
		gn_devinfo->sectorsize = 512;
		gn_devinfo->s_acccon= DEFAULT_ACCCON;
		gn_devinfo->s_acccon1= DEFAULT_ACCCON1;
		gn_devinfo->freq= 0x80;
		gn_devinfo->vendor = VEND_NONE;
		gn_devinfo->dqs_delay_ctrl = 0;
        return false;
	}
}
#ifdef DUMP_NATIVE_BACKTRACE
 ...
 *
 ******************************************************************************/
static void mtk_nand_configure_fdm(u16 u2FDMSize, u16 u2FDMEccSize)
{
...
static bool mtk_nand_dma_read_data(struct mtd_info *mtd, u8 * buf, u32 length)
{
    ...
    int ret = dma_map_sg(mtd->dev.parent, &sg, 1, dir);
    if(ret == 0)
    	printk("[%s]DMA mapping failed!!!\n", __func__);
#endif
#if !defined (CONFIG_SYS_DCACHE_OFF)
    invalidate_dcache_range((unsigned long)buf, (unsigned long)buf+length);
#endif
 ...
 *
 ******************************************************************************/
__attribute__((aligned(_SNAND_CACHE_LINE_SIZE))) static u8 fdm_buf[64];
...
#if !defined (CONFIG_SYS_DCACHE_OFF)
    if (((unsigned long) pPageBuf % 64) && local_buffer_16_align)
#else
...
static void mtk_nand_command_bp(struct mtd_info *mtd, unsigned int command, int column, int page_addr)
{
    ...
    switch (command)
    {
      ...
      case NAND_CMD_READID:
          ...
          mb();
          mtk_nand_set_mode(CNFG_OP_SRD);
          (void)mtk_nand_set_command(command);
      ...
      case NAND_CMD_GET_FEATURES:
          ...
          (void)mtk_nand_set_address(column, 0, 1, 0);
	  nand_wait_ready(mtd);
          DRV_WriteReg16(NFI_CON_REG16, 0x1000 | CON_NFI_BRD);
          while (DRV_Reg32(NFI_STA_REG32) & STA_DATAR_STATE) ;
          break;

      case NAND_CMD_SET_FEATURES:
          printk("in NAND_CMD_SET_FEATURES\n");
          /* Issue NAND chip reset command */
          mtk_nand_reset();
	  ...
	  //mtk_nand_set_mode(0x6000);
          //NFI_CLN_REG16(NFI_CNFG_REG16, CNFG_READ_EN);
          //NFI_SET_REG16(NFI_CNFG_REG16, CNFG_BYTE_RW);
	  DRV_WriteReg16(NFI_CNFG_REG16, 0x6040);
          (void)mtk_nand_set_command(command);
          (void)mtk_nand_set_address(column, 0, 1, 0);
	  while ((DRV_Reg32(NFI_STA_REG32) & 0xF) != 0x0) ;

	  DRV_WriteReg16(NFI_CON_REG16, 0x1000 | CON_NFI_BWR);
...
void mtk_nand_set_clk(void)
{
	int reg;

	/* Peripheral clock selection */
	/* Set Peribus operates at quarter clock */
	DRV_WriteReg32(0x1000205C, 0);

	/* CLK_CFG_0 */
	/* clk_axi_sel = 1: syspll1_d2_280MHz */
	reg = DRV_Reg32(0x10210040);
	reg = (reg & ~(0x7)) | 0x1;
	DRV_WriteReg32(0x10210040, reg);

	/* CLK_CFG_1 */
	/* clk_spinfi_infra_bclk_sel = 9: syspll1_d8_70MHz */
	reg = DRV_Reg32(0x10210050);
	reg = (reg & ~(0xf0000)) | 0x90000;
	DRV_WriteReg32(0x10210050, reg);
}

void mtk_nand_set_feature(struct nand_chip *nand_chip, struct mtd_info *mtd, u32 address, u8 *feature, u32 len)
{
	/* Set feature */
        nand_chip->select_chip(mtd, 0);
	nand_chip->cmdfunc(mtd, NAND_CMD_SET_FEATURES, address, -1);
	mtk_nand_mcu_write_data(mtd, feature, len);
        nand_chip->select_chip(mtd, -1);
}

void mtk_nand_get_feature(struct nand_chip *nand_chip, struct mtd_info *mtd, u32 address, u8 *feature, u32 len)
{
	int i;

	/* Get feature */
	memset(feature, 0x0, len);
        nand_chip->select_chip(mtd, 0);
	nand_chip->cmdfunc(mtd, NAND_CMD_GET_FEATURES, address, -1);
	for (i = 0; i < len; ++i)
	    feature[i] = nand_chip->read_byte(mtd);
        nand_chip->select_chip(mtd, -1);
}
...
#endif
{
	...
	mt_set_gpio_mode(GPIO40, GPIO_MODE_00);
    ...
    nand_chip->chipsize = (((uint64_t)gn_devinfo.totalsize) <<20);
#endif
...
unsigned int samsung_OTPRead(unsigned int PageAddr, void *BufferPtr, void *SparePtr)
{
    ...
    bm_swap(mtd, SparePtr, BufferPtr);
...
unsigned int samsung_OTPWrite(unsigned int PageAddr, void *BufferPtr, void *SparePtr)
{
    ...
    bm_swap(mtd, SparePtr, BufferPtr);
    mtk_nand_write_fdm_data((struct nand_chip *)mtd->priv, SparePtr, sec_num);
