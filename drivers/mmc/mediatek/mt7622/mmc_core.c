...
int mmc_part_config(int dev_num, u8 ack, u8 part_num, u8 access)
{
	struct mmc_host *host = mmc_get_host(dev_num);
	struct mmc_card *card = mmc_get_card(dev_num);
	int err = MMC_ERR_NONE;
	...
	cfg = (ack) << 6 |(part_num << 3) | (access & 0x7);

	err = mmc_switch(host, card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CFG, cfg);
        if (err != MMC_ERR_NONE) {
            MSDC_TRC_PRINT(MSDC_ERROR,("[SD%d] Switch partition access error (0x%x) failed\n", dev_num, cfg));
            return err;
        }
