...
#if defined (CONFIG_RTL8367_V142)
#include "rtl8367_v142/rtk_types.h"
#include "rtl8367_v142/port.h"
#include "rtl8367_v142/vlan.h"
#include "rtl8367_v142/rtk_switch.h"
#include "rtl8367_v142/rtk_error.h"

#define MAC_FORCE  PORT_MAC_FORCE

#else
#include "rtl8367c/include/rtk_types.h"
...
#include "ar8337n/athrs17_phy.h"
#endif

#ifdef CONFIG_RTK_CPU_PORT
#define RTK_CPU_PORT CONFIG_RTK_CPU_PORT
#else
#define RTK_CPU_PORT EXT_PORT0
#endif
...
typedef enum _EXTERNAL_SWITCH_TYPE
{
	EXTERNAL_SWITCH_TYPE_RTK_RTL8367S,
	EXTERNAL_SWITCH_TYPE_QCA_AR8337N,
	EXTERNAL_SWITCH_TYPE_RTK_RTL8367S_VB,
	EXTERNAL_SWITCH_TYPE_END
} EXTERNAL_SWITCH_TYPE;

typedef struct _EXTERNAL_SWITCH
{
	EXTERNAL_SWITCH_TYPE type;

	void (*switch_init_func)(void);
	void (*lan_wan_partition_func)(void);
	void (*port_isolation_func)(void);
	int (*switchChipProbe)(void);
...
void mt7626_rtl8367_vlan_setup(void);
void rtk_switch_port_isolation(void);
int rtl8367c_switch_chip_probe(void);
#define RTL8367C_CHIP_NUM			0x6367
void rtl8367c_ethifsys_sgmii_init(void);
#endif

#if defined(CONFIG_AR8337N)
void athrs17_ethifsys_sgmii_init(void);
#endif


static EXTERNAL_SWITCH g_external_switch_arr[] = {
#if defined (CONFIG_RTL8367)
{
#if defined (CONFIG_RTL8367_V142)
	.type =                   EXTERNAL_SWITCH_TYPE_RTK_RTL8367S_VB,
	.switch_init_func =       mt7622_rtl8367_sw_init,
	.lan_wan_partition_func = mt7626_rtl8367_vlan_setup,
	.port_isolation_func =    rtk_switch_port_isolation,
	.switchChipProbe = rtl8367c_switch_chip_probe,
	.sgmii_init = rtl8367c_ethifsys_sgmii_init,
#else
	.type =                   EXTERNAL_SWITCH_TYPE_RTK_RTL8367S,
	.switch_init_func =       mt7622_rtl8367_sw_init,
	.lan_wan_partition_func = mt7626_rtl8367_vlan_setup,
	.port_isolation_func =    rtk_switch_port_isolation,
	.switchChipProbe = rtl8367c_switch_chip_probe,
...
#if defined (CONFIG_AR8337N)
{
	.type =                   EXTERNAL_SWITCH_TYPE_QCA_AR8337N,
	.switch_init_func =       athrs17_init,
	.lan_wan_partition_func = NULL,
	.port_isolation_func =    athrs17_port_isolation_config,
	.switchChipProbe = athrs17_switch_chip_probe,
	.sgmii_init = athrs17_ethifsys_sgmii_init,
},
#endif
};

static EXTERNAL_SWITCH *g_external_switch = NULL;
...
static void mt7622_rtl8367_sw_init(void)
{
	...
	ret = rtk_port_macForceLinkExt_set(RTK_CPU_PORT, mode,&mac_cfg);
	ret = rtk_port_sgmiiNway_set(RTK_CPU_PORT, DISABLED);

	rtk_port_phyEnableAll_set(ENABLED);
#endif	/* CONFIG_RTL8367 */
}

static void mt7622_ethifsys_sgmii_init(int speed)
{
	unsigned int reg_value;
...
static void leopard_ethifsys_pinmux(void)
{
#define GE2_RGMII_GPIO	0x10211300
...
#if defined (CONFIG_GE1_SGMII_FORCE_2500)
static void leopard_switch_reset(void)
{
	unsigned int reg_value;
		/* set GPIO54 as GPIO mode */
#define GPIO_MODE2 0x10211330
...
static void leopard_rtl8367_sw_init(void)
{
#if defined (CONFIG_RTL8367)
	int ret = 100;
	unsigned int reg_value;
...
static void leopard_ethifsys_sgmii_init(void)
{
	unsigned int reg_value;
...
#if defined (CONFIG_GE1_ESW)
void ephy_led_enable(void)
{
#define GPIO_MODE_BASE 0x10217300
	unsigned int reg_value = 0;

	/* 10217310	GPIO_MODE1 (gpio 12/13/14/15) [31:16] = 0x1 */
	reg_value = RALINK_REG(GPIO_MODE_BASE + 0x10);
	reg_value &= 0x0000ffff;
	reg_value |= 0x11110000;
	RALINK_REG(GPIO_MODE_BASE + 0x10) = reg_value;

	/* 10217320	GPIO_MODE2(gpio16) [3:0] = 0x1 */
	reg_value = RALINK_REG(GPIO_MODE_BASE + 0x20);
	reg_value &= 0xfffffff0;
	reg_value |= 0x00000001;
	RALINK_REG(GPIO_MODE_BASE + 0x20) = reg_value;

	/* Fix P0 LED always on problem. */
...
static void leopard_ethifsys_esw_init(void)
{
	/* EPHY setting */
	*(volatile u_long *)(0x1b11000c) = 1;
	*(volatile u_long *)(0x1b118084) = 0x8000000;
	*(volatile u_long *)(0x1b11809c) = 0x0008a041;
	*(volatile u_long *)(0x1b110000) = 0xc5000100;
	*(volatile u_long *)(0x1b118090) = 0x7f7f;
	*(volatile u_long *)(0x1b1180c8) = 0x20503bfa;
	*(volatile u_long *)(0x1b000014) = 0x00110414;
	/* To CPU setting */
	*(volatile u_long *)(0x1b11808c) = 0x02404040;
	/* vlan untag */
	*(volatile u_long *)(0x1b118098) = 0x7f7f;
	/* AGPIO setting */
	*(volatile u_long *)(0x10000710) = 0x10000820;
	/* gmac0 connect gphy*/
	/*bit[1]: gphy connect GMAC0 or GMAC2 1:GMAC0. 0:GMAC2*/
	/*bit[0]: Co-QPHY path selection 0:U3path, 1:SGMII*/
...
#if defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_ESW)
void mt7626_esw_vlan_setup(void)
{
#define RALINK_ETH_ESW_BASE 0x1b118000
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x14) = 0x405555;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x50) = 0x2001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x98) = 0x7f7f;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0xe4) = 0x3f;
#if defined (CONFIG_WAN_AT_P0)
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x40) = 0x1002;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x44) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x48) = 0x2001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x70) = 0xffff615e; /* P0 connect to Port 5. */
	printf("CONFIG_WAN_AT_P0...DONE\n");
#elif defined (CONFIG_WAN_AT_P1)
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x40) = 0x2001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x44) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x48) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x70) = 0xffff427d;
	printf("CONFIG_WAN_AT_P1...DONE\n");
#elif defined (CONFIG_WAN_AT_P2)
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x40) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x44) = 0x1002;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x48) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x70) = 0xffff447b;
	printf("CONFIG_WAN_AT_P2...DONE\n");
#elif defined (CONFIG_WAN_AT_P3)
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x40) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x44) = 0x2001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x48) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x70) = 0xffff4877;
	printf("CONFIG_WAN_AT_P3...DONE\n");
#elif defined (CONFIG_WAN_AT_P4)
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x40) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x44) = 0x1001;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x48) = 0x1002;
	RALINK_REG(RALINK_ETH_ESW_BASE + 0x70) = 0xffff506f;
	printf("CONFIG_WAN_AT_P4...DONE\n");
#endif
}
#endif

#if defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_SGMII_FORCE_2500)
#if defined (CONFIG_RTL8367)

#if defined (CONFIG_WAN_AT_P0)
#define UTP_WAN_PORT UTP_PORT0
#elif defined (CONFIG_WAN_AT_P1)
#define UTP_WAN_PORT UTP_PORT1
#elif defined (CONFIG_WAN_AT_P2)
#define UTP_WAN_PORT UTP_PORT2
#elif defined (CONFIG_WAN_AT_P3)
#define UTP_WAN_PORT UTP_PORT3
#else
#define UTP_WAN_PORT UTP_PORT4
#endif
...
void mt7626_rtl8367_vlan_setup(void)
{
	...
	printf("CONFIG_WAN_AT_P%d...DONE\n", UTP_WAN_PORT);

	/* VLAN setting, 4 lan ports and ext_port 1 as one vlan */
	memset(&vlan, 0x0, sizeof(rtk_vlan_cfg_t));
	RTK_PORTMASK_PORT_SET(vlan.mbr, RTK_CPU_PORT);
	RTK_PORTMASK_PORT_SET(vlan.untag, RTK_CPU_PORT);
	...
	rtk_vlan_portPvid_set(RTK_CPU_PORT, 1, 0);
	for (i = UTP_PORT0; i <= UTP_PORT4; i++)
	{
		if (i == UTP_WAN_PORT)
		{
...
static void LANWANPartition(void)
{
	...
	IsSwitchVlanTableBusy();
#elif defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_ESW)
	mt7626_esw_vlan_setup();
#elif defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_SGMII_FORCE_2500)
	if (g_external_switch->lan_wan_partition_func)
	{
		g_external_switch->lan_wan_partition_func();
	}
#endif
}
#if defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_SGMII_FORCE_2500)
...
void rtk_switch_port_isolation(void)
{
	...
	for (port = UTP_PORT0; port <= UTP_PORT4; port++)
	{
		...
		RTK_PORTMASK_PORT_SET(pPortmask, RTK_CPU_PORT);
		rtk_port_isolation_set(port, &pPortmask);
	}
}
int rtl8367c_switch_chip_probe(void)
{
	...
	/* delay is essential! or rtk_switch_init will fail! */
	mdelay(1000);

#if defined (CONFIG_RTL8367_V142)
	if((switch_probe(&switchChip)) == RT_ERR_OK)
	{
		return 1;
	}
#else
	if((rtk_switch_probe(&switchChip)) == RT_ERR_OK)
	{
		return 1;
	}
#endif
...
#endif
#endif
void PortIsolation(void)
{
#if defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_ESW)
	/* TODO */
#elif defined(MT7626_ASIC_BOARD) && defined (CONFIG_GE1_SGMII_FORCE_2500)
	if (g_external_switch->port_isolation_func)
	{
		g_external_switch->port_isolation_func();
	}
#endif
}
...
#if defined (CONFIG_GE1_SGMII_FORCE_2500) /* 初始化SGMII，包括1G和2.5G(这个宏名字不对)*/
	int index = 0;
	for(index = 0 ;index < sizeof(g_external_switch_arr)/sizeof(g_external_switch_arr[0]); index++)
	{
		if(g_external_switch_arr[index].switchChipProbe())
		{
			g_external_switch = &g_external_switch_arr[index];
			break;
		}
	}

	if (g_external_switch->sgmii_init)		/* MT7622 SGMII Init */
	{
		g_external_switch->sgmii_init();
	}
#endif
	if (g_external_switch->switch_init_func)
	{
		g_external_switch->switch_init_func();
	}
